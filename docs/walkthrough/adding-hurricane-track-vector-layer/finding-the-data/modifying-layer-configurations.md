...
As we saw in \[Modifying layer configurations\]\(INSERT LINK HERE\) for WMTS Image layers, all non-default layer primarily resides in `layers.json` . So let's go ahead and add a new layer entry for this storm track.
